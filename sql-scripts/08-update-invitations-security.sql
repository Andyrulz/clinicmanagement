-- Update invitations table and add security functions
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'invitations' AND column_name = 'role') THEN
        ALTER TABLE invitations ADD COLUMN role user_role NOT NULL DEFAULT 'staff';
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'invitations' AND column_name = 'accepted_by') THEN
        ALTER TABLE invitations ADD COLUMN accepted_by UUID REFERENCES users(id) ON DELETE SET NULL;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'invitations' AND column_name = 'updated_at') THEN
        ALTER TABLE invitations ADD COLUMN updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW();
    END IF;
END $$;
...
DROP POLICY IF EXISTS "Authenticated users can view invitations in their tenant" ON invitations;
...
-- Verify the updates
SELECT 'Invitations security functions updated successfully!' as result;
...
SELECT 'create_invitation_secure function exists: ' || 
       CASE WHEN EXISTS (
         SELECT 1 FROM pg_proc p 
         JOIN pg_namespace n ON p.pronamespace = n.oid 
         WHERE n.nspname = 'public' AND p.proname = 'create_invitation_secure'
